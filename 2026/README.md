# Introducción a la genómica (Edición 2026)

Desde el departamento de Life Sciences, la **sesión de Introducción a la genómica** del **módulo 7 sobre Biología computacional** va a cargo de **Anna Costa Garrido y Laura Figueiredo Tor**.

:pushpin: **Día**: Sábado 7 de Marzo 2026

:pushpin: **Horario**: 9.30h a 14h 

:pushpin: **Lugar**: Auditorio BSC

:pushpin: **Idioma**: Castellano / Catalán


### Links para la sesión

 - [Slides 2026](https://docs.google.com/presentation/d/1f3slfqGkz7WKXXW6QvoQjK1SiqXNfTZurYXDiV-GLDs/edit?usp=sharing)
 - Archivos necesarios para las actividades:
    - *Ejercicio 1*: Dogma Central de la Biología Molecular
      - [Jupyter notebook](Bojos2026_Exercici1_DogmaCentralBio.ipynb)
    - *Ejercicio 2*: SNPs y riesgo de enfermedad
      - [Jupyter notebook](Bojos2026_Exercici2_SNPs.ipynb)
      - [Datos SNPs](data/snps_data.txt)
    - *Ejercicio 3*: Identificar genes diferencialmente expresados (DEGs)
      - [Jupyter notebook](Bojos2026_Exercici3_DEGs.ipynb)
      - - [Datos DEG](data/degs_data.txt)
    - *Ejercicio 4*: Aplicaciones biológicas de Machine Learning
      - [Jupyter notebook](Bojos2026_Exercici4_ML.ipynb)    
      - [Niveles de expresión](data/breast_cancer_data.csv)
      - [Sequencia DNA tumoral](data/breast_cancer_classification.csv)

      
### Calendario

| Hora  |                                                                            |
|-------|----------------------------------------------------------------------------|
| 9:30  | Presentación                                                               |
| 9:45  | **Primera parte**: Conceptos clave sobre genómica **Guillermo y Moisés**   |
| 10:15 | DEBATE: ¿Para qué necesitamos los biólogos los superodenadores? :computer: |
| 10:30 | 20 min BREAK :coffee: :croissant:                                          |
| 10:50 | **Segunda parte**: Conceptos sobre python **Guillermo Prol**               |
| 11:20 | Actividad 1: *Diagnóstico de cáncer con IA*                                |
| 12:30 | 10 min BREAK :toilet:                                                      |
| 12:40 | **Tercera parte**: Conceptos clave sobre filogénia **Moisès Bernabeu**     |
| 13:00 | Actividad 2: *Origen de un gen importante para el desarrollo de cáncer*    |
| 13:50 | RESUMEN Y DEBATE SOBRE EL FUTURO DE LA BIOLOGÍA COMPUTACIONAL              |
| 14:00 | FIN de la clase                                                            |
